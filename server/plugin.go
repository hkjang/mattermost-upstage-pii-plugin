...
func (p *Plugin) OnActivate() error {
	...
	if err := p.ensureBots(); err != nil {
		p.API.LogError("Failed to ensure PII inference bots during activation", "error", err)
	}
...
func (p *Plugin) MessageHasBeenPosted(_ *plugin.Context, post *model.Post) {
	...
	go func() {
		if err := p.handlePostedMessage(post); err != nil {
			p.API.LogError("Failed to process PII inference post trigger", "error", err, "post_id", post.Id)
		}
	}()
}
...
func buildBotPromptMessage(bot BotDefinition) string {
	lines := []string{
		fmt.Sprintf("`@%s` 는 PII Inference `%s` 를 사용합니다.", bot.Username, bot.Model),
		"",
		"문서 파일을 보내면 개인정보 필드를 추출하고, 마스킹 옵션이 켜져 있으면 민감한 값이 가려집니다.",
		"메시지 텍스트는 PII API로 전달되지 않으며, vLLM 후처리를 설정한 경우에만 추가 지시로 사용됩니다.",
		"",
		"예시:",
	}
