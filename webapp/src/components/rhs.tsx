...
export default function RHSPane() {
    ...
    async function submit() {
        ...
        try {
            ...
            setMessage(`@${bot.username} 봇이 Mattermost 스레드에 PII 추출 결과를 게시했습니다.`);
        } catch (e) {
            setMessage((e as Error).message);
        } finally {
            setSubmitting(false);
    ...
    return <div style={{display: 'flex', flexDirection: 'column', gap: 16, padding: 16}}>
        <section style={card}>
            <strong>{'Upstage PII Masking'}</strong>
            <span style={{fontSize: 12, opacity: .8}}>{'현재 선택한 포스트의 첨부 파일만 PII API의 document 파트로 전송합니다. 메시지 텍스트는 vLLM 후처리를 켠 경우에만 사용됩니다.'}</span>
            {loading && <span>{'봇 목록을 불러오는 중입니다...'}</span>}
            {!loading && bots.length === 0 && <span>{'현재 채널에서 사용할 수 있는 PII 봇이 없습니다.'}</span>}
            {!loading && bots.length > 0 && <>
                ...
                <div style={{fontSize: 12, opacity: .8}}>{`Model: ${bot?.model || 'pii'} | schema=${bot?.schema || 'oac'} | lang=${bot?.lang || 'auto'} | verbose=${bot?.verbose ? 'true' : 'false'}${bot?.vllm_model ? ` | vLLM=${bot.vllm_model}` : ''}`}</div>
                {bot?.description && <span style={{opacity: .8}}>{bot.description}</span>}
                <div style={{fontSize: 12, opacity: .8}}>{selectedPostId ? (fileNames.length > 0 ? `첨부 파일: ${fileNames.join(', ')}` : '첨부 파일이 없습니다. 파일이 있는 포스트를 선택해 주세요.') : '포스트에서 RHS를 열면 해당 첨부 파일을 바로 PII 추출할 수 있습니다.'}</div>
                <textarea style={{...field, resize: 'vertical'}} rows={5} value={prompt} placeholder={'예: 주민번호와 주소만 간단히 요약해줘 (vLLM 후처리용 선택값)'} onChange={(e) => setPrompt(e.target.value)}/>
                <button className='btn btn-primary' type='button' disabled={submitting || !bot || !channelId || fileIds.length === 0} onClick={submit}>{submitting ? 'PII 추출 요청 중...' : `@${bot?.username || 'bot'}로 실행`}</button>
            </>}
